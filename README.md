...
I keep the two models on the same amenity, then the objects that sit next to them in the assessment corpus: the Alonso–Muth–Mills envelope *R'(d) = −t/q*, Roback *(w, R)* cells, housing-supply incidence, place-based jobs versus stayers, Tiebout's conditions as a set, and a constructed hedonic omitted-variable. Cities here are constructed. There is no ACS extract.
...
## Same amenity, two residual claimants
...
Constructed cities. Not ACS, not a Saiz instrument, not a school-district hedonic. The open-city pin is a scalar \(\bar u\), not a migration model.
